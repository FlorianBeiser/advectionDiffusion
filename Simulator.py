...
As spatio-temporal model we separate it into the underlying grid 
...
class Grid:
    ...
    def __init__(self, nx, ny, cell_size):
        ...
        self.x_grid = np.arange(0,self.nx) * self.cell_size
        self.y_grid = np.arange(0,self.ny) * self.cell_size
        ...
        self.H = distance_matrix(df.values, df.values)
...
class Simulator:
    def __init__(self, grid, D=0.05, v=np.array([0.5,0.1]), zeta=-0.0001, dt=0.01, noise=0.1):
        ...
        """
        self.grid = grid
        self.A = self.matrix(D, v, zeta, dt)
    ...
    def forecasting(self, mean, cov):
        forecasted_mean = np.matmul(self.A, mean)
        forecasted_covariance = np.matmul(self.A,np.matmul(cov, self.A.transpose())) + self.noise
...
class Observation:
    ...
    def matrix(self):
        self.G = np.zeros((self.N_y, self.grid.N_x))
        for i in range(self.N_y):
            self.G[i,self.obsidx[i]] = 1 
