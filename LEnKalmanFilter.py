...
class LEnKalman:
    ...
    def initializeLocalisation(self, scale_r):
        ...
        for d in range(self.N_y):
            ...
            self.all_Ls[d], self.all_xrolls[d], self.all_yrolls[d] = \
                LEnKalman.getLocalIndices(self.observation_positions[d], scale_r, \
                        dx, dy, nx, ny)

        self.W_loc = LEnKalman.getLocalWeightShape(scale_r, dx, dy, nx, ny)

        W_combined = LEnKalman.getCombinedWeights(self.observation_positions, scale_r, dx, dy, nx, ny, self.W_loc)
    ...
    def getLocalWeightShape(scale_r, dx, dy, nx, ny):
        ...
        for y in range(local_ny):
            for x in range(local_nx):
                loc = np.array([(x+0.5)*dx, (y+0.5)*dy])
                weights[y,x] = min(1, LEnKalman.distGC(obs_loc, loc, scale_r*dx, nx*dx, ny*dy))
    ...
    def getCombinedWeights(observation_positions, scale_r, dx, dy, nx, ny, W_loc):
        ...
        W_loc = LEnKalman.getLocalWeightShape(scale_r, dx, dy, nx, ny)
        
        for d in range(num_drifters):
            # Get local mapping for drifter 
            L, xroll, yroll = LEnKalman.getLocalIndices(observation_positions[d,:], scale_r, dx, dy, nx, ny)
